...
class MocapOdomWrapper:
    def __init__(self, quad_name, odom_rate=100, simulate_noise=False, use_ekf=False,msg_type='Odometry'):
        ...
        base_computer_odom_topic = "/" + quad_name + "/odometry"

        # Publishers
        self.msg_type = msg_type
        if msg_type == 'QuadState':
            self.odom_pub = rospy.Publisher(odom_topic, QuadState, queue_size=10, tcp_nodelay=True)
            self.base_computer_odom_pub = rospy.Publisher(base_computer_odom_topic, QuadState, queue_size=10, tcp_nodelay=True)
        elif msg_type == 'Odometry':
            self.odom_pub = rospy.Publisher(odom_topic, Odometry, queue_size=10, tcp_nodelay=True)
            self.base_computer_odom_pub = rospy.Publisher(base_computer_odom_topic, Odometry, queue_size=10, tcp_nodelay=True)
        else:
            print("Invalid msg type selected: %s"%msg_type)
            return
        ...
        while not rospy.is_shutdown():
            ...
            elif self.msg_type == 'QuadState':
                quad_state_msg = QuadState()
                quad_state_msg.header.stamp = rospy.Time.now()
                quad_state_msg.header.seq = self.seq_num
                quad_state_msg.header.frame_id = 'world'
                quad_state_msg.t = rospy.get_time()
                quad_state_msg.pose.position = Point(*state_means[:3])
                w, x, y, z = state_means[3:7]
                quad_state_msg.pose.orientation = Quaternion(x, y, z, w)
                quad_state_msg.velocity.linear = Vector3(*state_means[7:10])
                quad_state_msg.velocity.angular = Vector3(*state_means[10:13])

                # quad_state_msg.acceleration.linear = Vector3(*[0, 0, 0])
                # quad_state_msg.motors = []
            def _base_com_pub_thread_func():
                self.base_computer_odom_pub.publish(quad_state_msg)
            def _odom_pub_thread_func():
                self.odom_pub.publish(quad_state_msg)
            # self.base_com_pub_thread.join()
            self.base_com_pub_thread = threading.Thread(target=_base_com_pub_thread_func(), args=(), daemon=True)
            self.base_com_pub_thread.start()
            # self.odom_pub_thread.join()
...
def main():
    ...
    # Message Type: QuadState or Odometry
    msg_type = rospy.get_param('~msg_type', default='Odometry')
    
    MocapOdomWrapper(quad_name, odom_rate, simulate_noise=simulate_noise, use_ekf=use_ekf, msg_type=msg_type)
